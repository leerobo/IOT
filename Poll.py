...
# Toggle Pin
def PushGateway(SidType,sidTEMP,sidID):
    #sids=sid.split("#")
    #if sids[0] in config["SID"]:
    #    sidname=config["SID"][sids[0]] 
    #else:
    sidname=config["SID"][sidID] 
    #print(sids[0],sidname,sids[1])

    headers = {"Content-type":  "text/plain","version":"0.0.4"}
    url="/metrics/RPI/"+SidType
    JSx=[{"MID":config["RPI"]["name"],"LID":config["RPI"]["location"],"SID":sidname,"Value":str(sidTEMP) }]
    # print(JSx)
    try:
        Body = json.dumps(JSx) 
...
def main():
    ...
    while StopLoop==False:
        ...
        #sidv=[]                       # Hold Values
        for k,v in SIDlist.items():
            sidtemp=Wire1CNTL("GET",k,"VAL")
            if sidtemp != SIDlist[k]:
               print(str(sidtemp)+' - '+k+' - '+str(SIDlist[k]))
               SIDlist[k]=sidtemp
               Rid=PushGateway("WIRE1",sidtemp,k)    # Wire1 Temp settings
